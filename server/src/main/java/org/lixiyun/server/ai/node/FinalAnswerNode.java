...
//                    .toolNames()    // 清空工具名称：避免模型识别到不必要的工具调用指令
    ...
    public Map<String, Object> apply(OverAllState state, RunnableConfig config) throws Exception {
        ...
        if (streamFlat != null) {
            ...
            // DB添加会话上下文信息
            Optional<Object> metadata = config.metadata(GraphConstant.CONVERSATION_MESSAGES);
            List<Message> conversationMessages = (List<Message>) metadata.orElseThrow(() -> new BusinessException(ConversationExceptionEnum.CONVERSATION_METADATA_NOT_CONFIGURED));
