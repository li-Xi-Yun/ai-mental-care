 ...
 * <p>
 * 通过 {@link TypeMapper#register(TypeMapper.Reference)} 向 Jackson 的类型映射表中注册
 * InputGraph 在 state 中传递的所有自定义 POJO 类型，使序列化时写入 {@code @type} 属性，
 * 反序列化时能正确还原为原始类型。
 * <p>
 * 注册类型范围：
 * <ul>
 *   <li>上下文层：{@code ConversationProcessContextBO} 及其依赖的实体类</li>
 *   <li>输入侧：{@code InputResult} 及其全部子结构（清洗、结构化、统计、归一化、历史摘要）</li>
 * </ul>
 *
 * @see SpringAIJacksonStateSerializer
 * @see TypeMapper.Reference
 */
public class InputStateSerializer extends SpringAIJacksonStateSerializer {

    /**
     * @param stateFactory 状态工厂，通常传入 {@code OverAllState::new}
     */
    public InputStateSerializer(AgentStateFactory<OverAllState> stateFactory) {
        super(stateFactory);
        registerTypes();
    }
     ...
     */
    public InputStateSerializer(AgentStateFactory<OverAllState> stateFactory, ObjectMapper objectMapper) {
