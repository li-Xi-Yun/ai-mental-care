...
import org.lixiyun.pojo.bo.conversation.EmotionAnalysisBO;
...
public class EmotionRecognitionModel {
    ...
    private ChatOptions buildOllamaCompanionOptions() {
        return OllamaChatOptions.builder()
                ...
                .numPredict(maxToken)           // 最大生成token数，覆盖完整JSON输出（情绪分析JSON最多200-300token）
                ...
                .numCtx(maxToken)              // 足够的上下文窗口，容纳prompt规则+用户输入，默认2048
...
//                    .thinkOption(ThinkOption.ThinkBoolean.ENABLED)
    ...
    private ChatOptions buildDashScopeCompanionOptions() {
        return DashScopeChatOptions.builder()
                .model(dashscopeModelName) // 可选：qwen3-14b-chat（精度更高）/qwen-turbo（性价比高）
                ...
                .maxToken(maxToken)             // 足够容纳标准版情绪分析的完整JSON（多字段、100字分析文本）
...
//                    .tools(null)               // 清空工具列表，专注情绪分析
    ...
    private ChatOptions buildDeepSeekCompanionOptions() {
        return DeepSeekChatOptions.builder()
                .model(deepseekModelName) // 推荐使用deepseek-chat（对话型），也可使用deepseek-reasoner（推理型）

                .temperature(0.2)          // 低随机性（0-2），确保情绪标签/置信度识别一致，避免乱输出
                .topP(0.85)                // 聚焦高概率词汇（0-1），减少无意义内容，保证分析文本贴合青年语境
                .maxTokens(maxToken)            // 足够容纳完整JSON输出（标准版情绪分析最多600token，留冗余）
...
//                    .internalToolExecutionEnabled(false)
    ...
    public AssistantMessage call(ChatModel chatModel, String userPrompt) throws GraphRunnerException {
        return reactAgentBuilder(chatModel)
                    ...
                    .outputType(EmotionAnalysisBO.class)
                    .build()
                .call(userPrompt);
    }
