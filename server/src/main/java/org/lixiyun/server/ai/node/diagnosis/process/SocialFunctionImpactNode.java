...
public class SocialFunctionImpactNode implements NodeActionWithConfig, NodeExecutionSummary {
    ...
    public Map<String, Object> apply(OverAllState state, RunnableConfig config) throws Exception {
        ...
        if (diagnosisDataRequestOpt.isEmpty()) {
            log.error("诊断处理侧-社会功能影响评估-诊断数据为空");
        ...
        if (diagnosisDataOpt.isEmpty()) {
            log.error("诊断处理侧-社会功能影响评估-诊断结果数据为空");
    ...
    private String buildUserPrompt(InputResult inputResult, KnowledgeRetrieveResult knowledgeRetrieveResult) {
        ...
        if (knowledgeRetrieveResult != null && knowledgeRetrieveResult.getDiagnosisReferencePrompt() != null
                && !knowledgeRetrieveResult.getDiagnosisReferencePrompt().isBlank()) {
        ...
        sb.append("请评估并输出社会功能影响评估结果，包括社会功能受损程度、受影响的具体领域和对日常生活影响的描述。\n");
        return sb.toString();
    }
